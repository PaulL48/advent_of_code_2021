...
fn main() {
    ...
    let (index, last_called) = get_last_winning_index(&called_numbers, &mut boards).unwrap();

    println!(
        "Last winning board score: {}; Called on: {}; Product: {}",
        boards[index].score(),
        last_called,
        boards[index].score() * last_called
    );
}

fn get_last_winning_index(
    called_numbers: &[&str],
    boards: &mut [Board<BOARD_WIDTH, BOARD_HEIGHT>],
) -> Option<(usize, i32)> {
    let mut boards_remaining: HashSet<usize> = (0..boards.len()).collect();
    ...
    for number in called_numbers {
        ...
        for (board_index, board) in boards.iter_mut().enumerate() {
            if let BoardState::Win = board.call_number(number, numeric) {
                if boards_remaining.remove(&board_index) {
                    if boards_remaining.len() == 0 {
                        return Some((board_index, numeric));
                    }
                }
            }
        }
